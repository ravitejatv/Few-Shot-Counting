...
def NegativeLoss(output,boxes, use_gpu=True):
  Loss = 0.
  if boxes.shape[1] > 1:
    boxes = boxes.squeeze()
    for tempBoxes in boxes.squeeze():
      tempBoxes = tempBoxes.clip(min = 0)
      tempBoxes = tempBoxes.clip(max = 1)
      for outBoxes in output.squeeze(0):
        base = torch.from_numpy(np.ones(outBoxes.shape))
        if use_gpu: base = base.cuda()
        up_pad = (outBoxes.shape[1] - tempBoxes.shape[1])//2
        side_pad = (outBoxes.shape[0] - tempBoxes.shape[0])//2
        base[side_pad:tempBoxes.shape[0]+side_pad,up_pad:tempBoxes.shape[1]+up_pad] = tempBoxes
        result = base*outBoxes
        result[result<0] = 0
    ...
    boxes = boxes.clip(max = 1)
    for outBoxes in output.squeeze(0):
      base = torch.from_numpy(np.zeros(outBoxes.shape))
      if use_gpu: base = base.cuda()
      up_pad = (outBoxes.shape[1] - boxes.shape[1])//2
      side_pad = (outBoxes.shape[0] - boxes.shape[0])//2
      base[side_pad:boxes.shape[0]+side_pad,up_pad:boxes.shape[1]+up_pad] = boxes
      result = base*outBoxes
      result[result<0] = 0
      tempLoss = torch.sum(result)
      Loss += torch.sum(result)
  return Loss
