...
from utils import MincountLoss, PerturbationLoss, NegativeLoss
...
parser.add_argument("-wm", "--weight_mincount", type=float,default=1e-2, help="weight multiplier for Mincount Loss")
parser.add_argument("-wp", "--weight_perturbation", type=float,default=1e-2, help="weight multiplier for Perturbation Loss")
parser.add_argument("-wn", "--weight_negative", type=float,default=1e-4, help="weight multiplier for Negative Loss")

# parser.add_argument("-wm", "--weight_mincount", type=float,default=1, help="weight multiplier for Mincount Loss")
# parser.add_argument("-wp", "--weight_perturbation", type=float,default=10, help="weight multiplier for Perturbation Loss")
# parser.add_argument("-wn", "--weight_negative", type=float,default=1e-2, help="weight multiplier for Negative Loss")
...
negative_dir = args.data_path + 'mask_images'
...
for im_id in pbar:
    ...
    negimage = Image.open('{}/{}'.format(negative_dir, img_id+"_anno.png"))
    negimage.load()
    sample = {'image': negimage, 'lines_boxes': list()}
    sample = Transform(sample)
    negimage = sample['image']

    if use_gpu:
        image = image.cuda()
        boxes = boxes.cuda()
        negimage = negimage.cuda()
    ...
    if not args.adapt:
        ...
        for step in range(0, args.gradient_steps):
            ...
            lNegative = args.weight_negative * NegativeLoss(output,negimage)
            Loss = lCount + lPerturbation  + lNegative
    ...
    ### 2.2
    ##########################################
    gt_cnt_arr.append(gt_cnt)
    pred_cnt_arr.append(pred_cnt)
    kaggle_arr.append([im_id,pred_cnt])

    over_count = pred_cnt - gt_cnt
    under_count = gt_cnt - pred_cnt
    
    if len(count_errors1) < 5:
      count_errors1.append([im_id, over_count])
    else:
      mi = count_errors1[0][1]
      ind = 0
      for indi, i in enumerate(count_errors1):
        if mi>i[1]:
          mi = i[1]
          ind = indi
      
      if over_count>mi:
        count_errors1[ind] = [im_id, over_count]

    if len(count_errors2) <5:
      count_errors2.append([im_id, under_count])
    else:
      mi = count_errors2[0][1]
      ind = 0
      for indi, i in enumerate(count_errors2):
        if mi>i[1]:
          mi = i[1]
          ind = indi
      
      if under_count>mi:
        count_errors2[ind] = [im_id, under_count]

    np.save("gt_cnt_arr.npy", gt_cnt_arr)
    np.save("pred_cnt_arr.npy", pred_cnt_arr)
    np.save("count_errors1.npy", count_errors1)
    np.save("count_errors2.npy", count_errors2)
    np.save("kaggle_arr.npy", kaggle_arr)
    ##########################################
    ### 2.2
    ##########################################
